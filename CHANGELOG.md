...
- **`apps/web/src/routes/_authed.tsx`** — header now renders a "Connected to Quran.com" `Badge` (QF brand green `#0E7C5C` + green dot) beside the role badge when `useQfStatus().data?.connected === true`. Hidden for disconnected users; the `/today` banner remains their CTA.
- **Today → Home rename (label-only)**. `apps/web/src/components/AppSidebar.tsx` nav item now reads `Home / الرئيسية`. `apps/web/src/routes/_authed.today.tsx` hero H1/H2 updated to match. The route path stays `/today` (no churn for `qfAuth` callback redirect, `homeRouteForRole`, banner navigation, etc. — precedent ADR 0025).
- **ADR 0041** (toolbar restructure), **ADR 0042** (Ḥusary CDN audio choice + fallback semantics), **ADR 0043** (label-only Today→Home rename, path stable).

### Added — M8 QF User APIs: OAuth + Bookmarks + Goals + Streak swap (ADR 0040)
- **OAuth2 Authorization Code + PKCE flow** — new `apps/server/src/routes/qfAuth.ts` with `POST /authorize`, `GET /callback`, `GET /status`, `POST /disconnect`. PKCE verifier rides the OAuth round-trip inside an HMAC-signed `state` parameter (`{ userId, verifier, exp, nonce }`) so the flow works in dev over HTTP without cross-origin cookie complications. Tokens upserted into `qf_user_token` (table already existed from migration 0005). Scopes requested: `openid offline_access profile bookmark goal streak.read reading_session.create`.
- **`apps/server/src/qf/userTokens.ts`** (NEW) — DB-persisted token cache. `getUserAccessToken(userId)` refreshes via the `refresh_token` grant when within 60s of expiry, mirrors the in-memory cache pattern from `qf/tokens.ts`.
- **`apps/server/src/qf/bookmarks.ts`** (NEW) — `pushBookmark(userId, {surah, ayah})` fire-and-forget POST to QF Bookmarks. Wired into `commit_onboarding` finish (pushes the `inProgress` marker) and `submit_test` finish (pushes the highest covered ayah — "the student's frontier after this test"). Per DESIGN.md §13.6, errors are swallowed.
- **`apps/server/src/routes/qfUser.ts`** (NEW) — `GET /streak` returns the user's QF QURAN streak; `POST /goals` + `DELETE /goals/:qfGoalId` proxy to QF Goals API.
- **`apps/web/src/today/StreakBadge.tsx`** — streak swap. When connected to QF, the Today badge replaces the local `daily_streak()` value with the QF QURAN streak (`days` field), with a "via Quran.com" sublabel + QF accent color (`#0E7C5C`). Falls back silently to local on any QF error.
- **`apps/web/src/features/integrations/ConnectQuranComBanner.tsx`** (NEW) — prominent dismissible CTA banner at the top of `/today` for disconnected students. Quran.com-brand-aligned styling. Per-session dismissal via `sessionStorage`.
- **`apps/web/src/features/integrations/ConnectQuranCom.tsx`** (NEW) — Settings management card. Disconnected → same Connect button; connected → status pill + scope chips (Bookmarks · Goals · Streak · Reading Sessions) + token-expiry hint + Disconnect button.
- **`apps/web/src/features/integrations/useQfStatus.ts`** (NEW) — shared `useQfStatus` query + `startQfConnect` / `disconnectQf` / `getScopeChips` helpers.
- **`apps/web/src/routes/_authed.goals.tsx`** — rewrite from EmptyState to a real Goals page. List + create/edit/delete via `goal` table under existing RLS. Local row is source of truth; new goals fire-and-forget to QF Goals API and the local row's `qf_goal_id` is patched on success. Quran.com badge on synced rows.
- **`apps/web/src/features/goals/GoalFormModal.tsx`** (NEW) — title + description + target_date + target_pages (QF-only) form.
- **`apps/web/src/routes/_authed.today.tsx`** — `validateSearch` parses `?qf=connected|error` after the OAuth round-trip; fires a toast then clears the param.
- **ADR 0040** captures the PKCE flow shape, stateless state encoding rationale, scope choices, streak-swap UX, and the fire-and-forget rule.

### Added — M7.5 Settings page + Edit Memorization (ADR 0039)
- **`apps/web/src/routes/_authed.settings.tsx`** — full rewrite from EmptyState stub. Student layout: Profile (read-only), Connect Quran.com (delegates to `ConnectQuranCom`), Daily capacity (two NumberInputs mirroring `Step3Sessions.tsx` bounds — half-page allowed for new), Hifz direction (two-card picker reusing the `Step1Path.tsx` DIRECTION_OPTIONS pattern), Memorization status (has_completed_quran Switch + Edit Memorization button). Teacher layout: Profile only. All toggle writes are direct `supabase.from(...).update()` under existing `student_settings_self_rw` + `app_user_self` RLS — zero new endpoints, zero new SQL.
- **Edit Memorization** — `/onboarding?edit=1` is now a real entrypoint. `validateSearch` parses `edit: 1`; the `beforeLoad` gate is relaxed when in edit mode so `onboarding_complete=true` users can re-enter. `apps/web/src/onboarding/state.ts` gains a `HYDRATE` action that shallow-merges a `Partial<OnboardingState>`.
- **`apps/web/src/onboarding/hydrate.ts`** (NEW) — derives the pre-population patch from current DB state: `student_settings` for capacity + direction; `memorization_page` rows filtered to `memorized|mastered` joined to `quran-index.json` to derive fully-memorized juzs and surahs. Partial-surah `upToAyah` is deliberately not reconstructed (documented in ADR 0039); user re-ticks if needed. The Step 2 picker opens pre-filled.
- **`apps/web/src/routes/onboarding.tsx`** — edit mode skips Step 1, lands directly on Step 2 with `path='partial'` forced. Submit invalidates `['session']` + `['onboarding-hydrate']` and navigates back to `/settings` with a success toast. `commit_onboarding` is idempotent (ON CONFLICT DO NOTHING, migration 0013) so no data is lost on re-edit.
- **ADR 0039** captures the direct-write architecture, Edit Memorization route shape, HYDRATE action, and the partial-surah scope cut.

### Added — M7 polish foundation + cuttable polish (M7 polish line)
- **`@mantine/notifications` ^7.15.1** added to `apps/web/package.json`; `<Notifications position="top-right" />` mounted in `main.tsx`.
- **`apps/web/src/lib/toast.ts`** (NEW) — `toastError(err, title?)` + `toastSuccess(message, title?)` + `toastInfo` wrappers with consistent palette (red / sage / honey).
- **`apps/web/src/components/SkeletonRow.tsx`** (NEW) — shared `<SkeletonRow variant="card" | "list" />` primitive used by Settings, Goals, and integration cards.
- **Toast wiring** on the highest-value mutations: `TestCreationModal` (test create), `LiveTestRoute` (logError + finishTest), `_authed.classroom.tsx` (leaveMutation + enroll_via_code), `_authed.students.index.tsx` (group delete + Move-to-group + invite-code rotate), `SessionPlanCard` (load_next_session). Mantine notifications now surface error messages instead of silent failures.
- **PWA stub** — `apps/web/public/manifest.webmanifest` + `apps/web/public/icon.svg` (SVG icon with bilingual mihrab gradient + ت glyph). `index.html` gains `<link rel="manifest">` + apple-touch-icon + apple-mobile-web-app-* metas. Theme color updated from `#1f2937` to `#15351E` (mihrab.9). No service worker yet — installable, not offline-capable.
- **Mobile sweep** — `AppSidebar.module.css` gains a `@media (max-width: 640px)` block reducing hover-expand width to 220px and padding the nav items. `LiveTestRoute` two-pane grid collapses to single-column at ≤900px so the mushaf + error log stack vertically on phones.
