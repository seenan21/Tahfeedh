...
| `_authed.students.$studentId.tsx` | Teacher drill-in (M6, ADR 0027): header + 3 reused M7 cards via `studentId` prop + recent tests list + "Start test for this student" → `TestCreationModal` in `mode="enrolled_teacher"`. Sibling of `_authed.students.index.tsx` under `_authed` (ADR 0031) | Touching the teacher's per-student dashboard |
