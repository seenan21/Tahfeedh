-- 0016_post_test_pipeline.sql
-- Phase D (M4): post-test transactional writer.
--
-- submit_test(p_test_id, p_payload) closes a test that is in_progress, touches
-- ayah_review_state for every covered ayah, upserts error_location_stats from
-- the streamed error_log rows, decays untouched stats, promotes pages on
-- strong_pass, and returns a {new, recurring, cleared} summary for the
-- post-test summary screen.
--
-- ADR 0017 — single fat SQL fn (mirrors commit_onboarding from 0013).
-- ADR 0018 — error_log is populated by streaming POST /api/tests/:id/error;
--            this function reads what's already there, no error payload here.
-- ADR 0011 — error stats are merged at render time on the client; this fn
--            only maintains the canonical occurrence_count + cleared flag.
-- ADR 0015 — status only promoted via this pipeline (no manual marking).
--
-- Deferred to M5 (see -- TODO M5 markers below):
--   - memorized → mastered promotion (needs per-page consecutive_clean rollup)
--   - fail-downgrade (memorized → in_progress on rating='fail')
--   - last_reviewed_session_number + recent_stage / ready_at machine
--   - QF Bookmarks sync (M8)

-- ----------------------------------------------------------------------------
-- Payload shape (jsonb):
-- {
--   "rating":        "strong_pass" | "pass_needs_practice" | "excellent" |
--                    "good" | "needs_work" | "fail",
--   "notes":         "text" | null,
--   "coveredAyahs":  [{"surah":int, "ayah":int}, ...],   -- expanded by Express
--   "coveredPages":  [int, ...]                          -- pages fully inside coveredAyahs
-- }
-- ----------------------------------------------------------------------------
...
begin
  ...
  --    DISTINCT ON (signature) so logging the same pattern twice in one test
  --    only counts as a single occurrence of that pattern for this test.
  ...
  --    (DESIGN.md §12 line 690, §13.4 line 1137). Capture the rows that
  --    just transitioned to cleared for the summary.
  with covered as (
  ...
  -- TODO M5: memorized→mastered when consecutive_clean revision tests >= N.
  -- TODO M5: rating='fail' downgrade (memorized → in_progress).
