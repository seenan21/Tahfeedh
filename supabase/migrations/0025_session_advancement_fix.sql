-- 0025_session_advancement_fix.sql
-- ADR 0037 — Two fixes the user reported in the live session:
--
-- (1) Bug: passing a new-lesson test on page X did not advance the new-lesson
--     slot. Two root causes:
--     (a) submit_test promotion was UPDATE WHERE status='in_progress'. Only the
--         single onboarding-seeded inProgress page has that row; pages reached
--         by walking the frontier (Queue 1) have no memorization_page row, so
--         the UPDATE matched zero rows and the page was never promoted to
--         'memorized'. Queue 1 then returned page X again on the next session.
--     (b) Even when the row existed, only `strong_pass` promoted. ADR 0037
--         relaxes this to "any pass" (strong_pass / excellent / good /
--         pass_needs_practice) — see ADR for rationale.
--
--     Fix: page promotion is now an UPSERT (insert if absent, update if present
--     but never downgrade a 'mastered' page) and triggers on any pass rating.
--     The Queue 2 stage machine for those ayahs also engages on any pass
--     (recent_stage = 1, ready_at = +1d) so the page enters the revision
--     pipeline regardless of whether it was a "strong" or "good" first pass.
--
-- (2) Bug: students who memorized pages during onboarding saw "No revision yet"
--     even though they had memorized many juz. Root cause: _compute_session_plan
--     Queue 2 requires `ars.recent_stage IS NOT NULL AND ars.ready_at <= now()`,
--     and Queue 3 requires `bool_and(ars.graduated_at IS NOT NULL)`. Onboarding
--     inserts ayah_review_state rows with recent_stage=NULL and graduated_at=NULL
--     (the stage machine only engages on a strong_pass test, per ADR 0024). So
--     onboarded pages fell into neither queue.
--
--     Fix: new Queue 2b — memorized pages whose ayahs have no stage/graduation
--     progression yet. Catches onboarding-memorized pages. Ordered by
--     page_number asc (a stable, intuitive default for "I haven't tested any of
--     these yet, where do I start?"). Filled after Queue 2 (active stages take
--     priority), before Queue 3 (graduated maintenance can wait — a never-tested
--     memorized page is more likely to have decayed than a graduated one).
--
-- Final revision ordering: Queue 2 (active stages, due) → Queue 2b (never tested)
-- → Queue 3 (graduated, weighted priority).

-- ---------------------------------------------------------------------------
-- submit_test rewrite (extends 0020)
-- ---------------------------------------------------------------------------
...
begin
  ...
  --
  --    Stage-machine change vs 0020: any pass on a newly_memorized test now
  --    advances ayahs to stage 1 (not only strong_pass). This keeps the new-
  --    lesson promotion in step with the page-status promotion below.
  ...
  select
    ...
    case
      ...
      when v_test.test_type <> 'revision' then aws.prev_stage  -- newly_memorized + fail/needs_work
  ...
  -- 6a. Page promotion: newly_memorized + any pass → memorized. UPSERT, so
  --     pages with no prior memorization_page row also get promoted. Never
  --     downgrade a 'mastered' page back to 'memorized'.
...
-- ---------------------------------------------------------------------------
-- _compute_session_plan rewrite (extends 0020) — adds Queue 2b.
-- ---------------------------------------------------------------------------
...
begin
  ...
  if v_q2_remaining > 0 then
    select coalesce(array_agg(mp.page_number order by mp.page_number asc), '{}'::int[])
      into v_q2b_pages
    from memorization_page mp
    where mp.student_id = p_student_id
      and mp.status in ('memorized'::memorization_status,
